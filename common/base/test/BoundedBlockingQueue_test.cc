 ...
 public:
      ...
      latch_(numThreads)
  {
    ...
    for (int i = 0; i < numThreads; ++i)
    {
      ...
      threads_.emplace_back(new WJ::Thread(
            std::bind(&Test::threadFunc, this), WJ::string(name)));
    }
  ...
  void run(int times)
  {
    ...
    for (int i = 0; i < times; ++i)
    {
      ...
      queue_.put(buf);
      printf("tid=%d, put data = %s, size = %zd\n", WJ::CurrentThread::tid(), buf, queue_.size());
    }
  }
 ...
 private:
  ...
  void threadFunc()
  {
    ...
    while (running)
    {
      ...
      printf("tid=%d, get data = %s, size = %zd\n", WJ::CurrentThread::tid(), d.c_str(), queue_.size());
      running = (d != "stop");
    }
  ...
  WJ::BoundedBlockingQueue<std::string> queue_;
...
void testMove()
{
  ...
  WJ::BoundedBlockingQueue<std::unique_ptr<int>> queue(10);
